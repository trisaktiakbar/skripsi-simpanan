...
$result_tersangka = mysqli_query($conn, "SELECT * FROM tersangka WHERE kode_laporan = '$kode_laporan' AND id = '$id' LIMIT 1");

if (!mysqli_fetch_assoc($result_tersangka)) {
    header('Location: ..');
} else {
    $result_tersangka = query("SELECT * FROM tersangka WHERE kode_laporan = '$kode_laporan' AND id = '$id' LIMIT 1")[0];

    $nama_lengkap = $result_tersangka['nama_lengkap'];
    $tempat_lahir = $result_tersangka['tempat_lahir'];
    $tanggal_lahir = $result_tersangka['tanggal_lahir'];
    $domisili = $result_tersangka['domisili'];
    $foto = $result_tersangka['foto'];
}
...
if (isset($_POST['submit'])) {
    ...
    if ($_FILES['foto']['name'] !== '') {
        if ($_FILES['foto']) {
            $foto = uploadFile($_FILES['foto'], '../../../app/data/foto/tersangka/');
        }
    }

    mysqli_query(
        $conn,
        "UPDATE tersangka SET
...
<head>
    ...
    <title>SIMPANAN | Tambah Tersangka</title>
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4">Detail Tersangka</h1>
                    <ol class="breadcrumb mb-4">
                        ...
                        <li class="breadcrumb-item active"><a href="<?= $BASE_URL ?>laporan/detail?id=<?= $code ?>" class="text-decoration-none">Tersangka</a></li>
                                                    ...
                                                    <div>
                                                        <label for="foto" class="w-100">
                                                            <div class="text-center">
                                                                <img id="foto-preview" src="<?= $foto === '' ? $BASE_URL . 'assets/img/foto-preview.png' : $BASE_URL . 'app/data/foto/tersangka/' . $result_tersangka['foto'] ?>" alt="" class="w-100 object-fit-cover w-100 rounded-3 border border-1 mb-3 cursor-pointer">
                                                            </div>
                                                        </label>
